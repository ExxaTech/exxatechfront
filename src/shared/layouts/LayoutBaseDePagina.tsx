...
export const LayoutBaseDePagina: React.FC<ILayoutBaseDePaginaProps> = ({ children, titulo, barraDeFerramentas, caminho, icone }) => {
      ...
      <Box padding={1} display="flex" alignItems="center" flexWrap="wrap" height={theme.spacing(smDown ? 2 : mdDown ? 3 : 4)} gap={1}>
        ...
        <Box display="flex" height={theme.spacing(2)}>
          <Breadcrumbs separator={<NavigateNextIcon />}>
            <Link href="/" > <Icon >home</Icon> Inicio</Link>
            <Link href={caminho}> <Icon>{icone}</Icon> {titulo} </Link>
          </Breadcrumbs>
        </Box>
